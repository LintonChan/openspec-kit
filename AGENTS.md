...
```
需求描述 ──▶ ① 四件套 ──[停]──▶ ② 评审产出物 ──[停]──▶ ③ 评审签字 ──[停]──▶ 下一个包
             proposal        prototypes/            review-notes
             design          NN-xxx.html（界面型）   状态=已签字(Rn)
             specs           NN-xxx.md  （策略型）
             tasks
```
...
|---|---|---|
| ① 四件套出完 | 报告产出了什么、「评审待定项」有哪几条，**确认本包的评审产出物类型（界面原型 / 策略说明，见第二节）**，然后**问用户是否现在出** | 不问就接着画；不问就开下一个包；**默认当成界面原型** |
...
### 动手前先分岔：这个包要的是界面原型，还是策略说明？

**不是每个变更包都有界面。** 纯服务端策略、算法规则、跨端契约、非功能约束这类包，画界面原型没有意义——评审要看的是规则怎么运转，不是页面长什么样。

**两类产物都是 `prototypes/NN-xxx/NN-xxx.html`**，都要回填三对注入标记、都保留顶部导航栏——视觉体系与发布方式全仓统一。**区别只在每个 section 里装什么。**

| 判据：**产出物里有没有用户或运营会直接看到的界面？** | 类型 | section 里装什么 |
|---|---|---|
| 有（用户端页面、管理后台页面） | **界面原型** | macos-window 外壳 + 右侧 360px prd-panel，走本节以下全部规则 |
| 没有（策略 / 算法 / 契约 / 非功能） | **策略说明** | 规则说明块，**不套 macos-window / prd-panel**（没有界面可装，套了只剩空壳）。四段结构与 UML 选型见 `config.yaml` 的 `rules.prototypes` |

- 类型应当在**拆包方案或 proposal 里已经标注**；没标注就**问用户，MUST NOT 自己默认成界面原型**
- 类型一经确认，**AI 不得自行改判**
- 策略型仍然**要读 SKILL.md**——它要用同一套主题、组件类名与注入流程，只是不套 macos-window

**以下 macos-window / prd-panel 相关规则只适用于「界面原型」。**
...
2. 顶部有 48px 固定导航栏，能在各 section 间切换

**第三条按类型分**：

3. **界面型** —— 每个 section 是 **macos-window 外壳 + 右侧 360px `prd-panel`** 的一一对应结构，不是一张光秃秃的页面
   **策略型** —— 每个 section 是规则说明块（**不得出现空的 macos-window / prd-panel 壳**），且四段内容齐全：输入与输出 / 规则正文 / 边界与异常 / 可配置项；图必须自包含，**不引外部 CDN**
