...
| `.data-table` 及子选择器（`.td-num` / `.td-mono` / `.td-actions` / `.act` / `.act-danger`） | **通用数据表格：列表页/宽表首选**（13px、表头不折行、行 hover、含操作列） | 题目列表 / 试卷列表 / 码表 等业务宽表 |
| `.usage-detail-table` 及子选择器 | 用量明细**窄表**（11px、表头大写、无列宽控制）——**仅用于用量日志类**，勿套业务宽表（会挤到折行） | 调用明细列表 |
...
| `.toc-sidebar` / `.toc-item` / `.toc-dot` / `.toc-mirror-tag` / `.toc-header` / `.toc-toggle` / `.toc-sidebar.collapsed` | 原型自身的 TOC 目录导航（不是产品里的 sidebar）；支持折叠（骨架含 toggle 按钮 + JS） | 多 section 原型 |
| `.proto-with-prd` | 原型自身的 PRD 旁注容器（原型图 + 右侧 PRD 面板并排） | 所有原型外壳 |
