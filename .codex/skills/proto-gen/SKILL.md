...
description: >
  ...
  也支持无界面的「策略说明」型产出（算法规则、契约、非功能约束）——同一套主题与注入流程，
  但不套 macOS 窗口与 PRD 面板，改为规则说明块 + 按需 Mermaid 图。
  关键词：生成原型、新建原型、写原型、画原型、做个原型、出个原型、新增一个 html、新建 html 原型、策略说明。
...
**不是每个变更包都有界面。** 纯服务端策略、算法规则、跨端契约、非功能约束这类包，套上 macos-window 只会留一堆空壳。

判据：**产出物里有没有用户或运营会直接看到的界面？**

| | 类型 | 每个 section 里装什么 |
|---|---|---|
| 有 | **界面型** | `macos-window` 外壳 + 右侧 360px `prd-panel`，走本文全部规则 |
| 没有 | **策略型** | **规则说明块，不套 `macos-window` / `prd-panel`** |

**两类的共同点**（策略型也必须满足）：

- 产物同为 `NN-xxx.html`，同样跑 `assets/inject-assets.mjs` 注入三对标记
- 同一套主题、同一套组件类名、顶部 48px 固定导航栏
- **铁律「内容必须有需求依据」同样适用**——规则也不许凭空加，答不上「需求哪一条写了这个」就不写

**两条铁律对策略型不适用**，因为它没有 canvas：

- ~~canvas 只放产品界面，评审说明一律进 panel~~ —— 策略型整篇都是说明，无需分离
- ~~交互链路必须闭环~~ —— 策略型没有可点击操作

**策略型的内容结构**（四段齐全：输入与输出 / 规则正文 / 边界与异常 / 可配置项）与 UML 选型规则，
由 openspec 侧的 `config.yaml` `rules.prototypes` 在起草时注入，本文不重复。图用 Mermaid，
**产物必须自包含，禁止引外部 CDN**。

> 类型应当在**拆包方案或 proposal 里已标注**。没标注就问用户，**不要默认成界面型**。
